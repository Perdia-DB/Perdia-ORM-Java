...
class CryptoTest {
    ...
    void encryptionTest() {
        ...
        Assertions.assertNotEquals(Arrays.hashCode(testBytes), Arrays.hashCode(c.encrypt(testBytes)));

        // Check if encrypt() works correctly by decrypting and asserting afterwards
        Assertions.assertEquals(Arrays.hashCode(testBytes), Arrays.hashCode(c.decrypt(c.encrypt(testBytes))));
    }

    @Test
    void decryptionTest() {
        // Check if decrypt actually changes something
        Assertions.assertNotEquals(Arrays.hashCode(c.encrypt(testBytes)), Arrays.hashCode(c.decrypt(c.encrypt(testBytes))));
